...
#                 src_img = get_image('/sda/rina_1921cs13/vipin/SourceImages/' + dataset + '/' + source[i][j+2]['image_name'])
                src_img = target_img
                pairImages.append(target_img)  #                pairImages.append([target_img, src_img])
                pairTexts.append([target_txt, src_txt])	
#                 ID.append([source[i][j+2]['image_name']])
                ID.append([source[i][1]])
                identity.append(ctr)			
                if target[i][5]=='FAKE':
                    pairLabels.append([0])
                else:
...
#            multimodal_concat = F.normalize(torch.cat((img_out, txt_out), 1), dim=1)
            multimodal_concat = F.normalize(txt_out, dim=1)
...
np.save('/sda/rina_1921cs13/vipin/vipin/data/source_unimodal_out.npy', source_all_out)
np.save('/sda/rina_1921cs13/vipin/vipin/data/target_unimodal_out.npy', target_all_out)
